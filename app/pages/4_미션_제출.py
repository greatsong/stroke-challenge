...
입력이름 = {"age": "나이", "avg_glucose_level": "평균 혈당", "bmi": "체질량지수",
            ...
            "smokes": "현재 흡연 여부", "self_employed": "자영업 여부", "male": "성별"}
...
고른열 = st.multiselect("입력으로 사용할 속성", 고를수있는열, default=기본열, format_func=우리말,
                        help="처음에는 나이·평균 혈당·고혈압·심장병 네 가지가 골라져 있습니다. "
                             "결혼 여부·현재 흡연 여부·자영업 여부는 예(1)·아니요(0)로 바꾼 속성이고, 성별은 남성이면 1, 그 밖에는 0입니다.")
...
for 이름 in 고른모델:
    ...
    줄들.append("| " + " | ".join(칸) + " |")
st.markdown("\n".join(줄들))
st.caption(f"train 30,380명 가운데 30%를 검증용으로 떼어 두고 나머지로 학습합니다. 검증 환자는 {int(y[검증용].sum())}명입니다. 제출할 모델은 아래에서 고릅니다. 굵은 줄은 검증 F2가 가장 높은 모델입니다. "
...
# 제출할 모델은 참가자가 고른다. 고른 모델 목록에서 빠졌을 때만 검증 F2가 가장 높은 모델로 다시 잡는다
if st.session_state.get("제출모델") not in 고른모델:
    st.session_state["제출모델"] = 좋은모델
제출모델 = st.radio("제출할 모델", 고른모델, key="제출모델", horizontal=True,
                    format_func=lambda 이름: 병기(이름) + (" · 검증 F2 최고" if 이름 == 좋은모델 else ""))
...
순위판주소 = "../leaderboard.html"                      # 실습실(app/)의 한 단계 위에 있는 순위판 페이지
...
if not 등록됨:
    ...
    st.write(f"고른 모델 {을를(병기(제출모델))} train {len(전체X):,}명 전체로 다시 학습해 test 13,020명의 확률을 구하고, "
    ...
    if st.button("이 설정으로 전화 목록 만들기", type="primary"):
        with st.spinner("train 전체로 다시 학습하고 있습니다."):
            ...
            학습결과 = 실험학습(제출모델, 설정키(제출모델), tuple(입력열), 결측처리, "전체")
            확률 = 확률구하기(학습결과, Xb)
        st.session_state["전화목록"] = {
            "ids": [int(v) for v in 시험.loc[확률 >= 내기준, "id"]], "전체": len(시험),
            "설정글": 설정글(제출모델, 내설정[제출모델]),
            "설정": {"model": 정식이름.get(제출모델, 제출모델), "inputs": " · ".join(우리말(열) for 열 in 입력열),
                     "missing": 결측처리, "threshold": float(내기준),
                     "params": dict(내설정[제출모델]),
                     "validation_f2": None if 내기록[제출모델]["F2"] is None else round(float(내기록[제출모델]["F2"]), 4)}}
