...
st.write("검증 F2 최고 기록과 제출 기록, 처음 설정에서 바꾼 것은 성능 높이기 페이지에서 그대로 가져옵니다. "
...
def 바꾼것(최고):
    ...
    if 최고["모델"] == "질문으로 답하는 모델" and 최고["깊이"] != 처음설정["깊이"]:
...
def 모델글(기록):
    return {"확률로 답하는 모델": "로지스틱 회귀", "질문으로 답하는 모델": "의사결정트리"}.get(기록["모델"], 기록["모델"])


def 설정글(기록):
    질문 = f" · 질문 {기록['깊이']}번" if 기록["모델"] == "질문으로 답하는 모델" else ""
...
st.subheader("검증 F2 최고 기록")
최고들 = st.session_state.get("최고기록") or 읽기(최고기록파일) or {}
if "F2" not in 최고들:
    st.warning("아직 기록이 없습니다. 성능 높이기 페이지의 챌린지에서 기록을 먼저 만듭니다.")
    st.stop()
...
st.caption("train 안 검증 데이터에서 F2가 가장 높았을 때의 설정입니다. 제출한 기록은 아래 표에 있습니다.")
제출들 = 읽기(기록폴더 / "제출.json") or []
if 제출들:
    st.subheader("제출 기록")
...
if 최고 is None:
    ...
    st.markdown(f"검증 최고 기록은 F2 {소수(최고['F2'])} · 찾아낸 환자 {최고['찾아낸 환자']}명 · 안내 {최고['안내 인원']:,}명 · "
                f"{모델글(최고)}입니다.")
    ...
    if not 내생각:
        ...
        채울것 = {
            "__RECORD__": " / ".join(f"{목표이름[목표]} {점수글(최고들[목표], 목표)}(찾아낸 환자 "
                                     ...
                                     for 목표 in 목표이름 if 목표 in 최고들),
            "__SETTING__": f"{모델글(최고)} · {설정글(최고)}" if 최고 else "기록 없음",
            "__CHANGED__": "\n".join(바꾼목록) or "기록 없음",
            "__REASON__": 내생각,
        }
        주소 = 제출주소
...
st.info("**이 흐름은 다른 데이터에도 그대로 됩니다** · 데이터를 소개하고, 들여다보고, 모델을 만들고, 검증하고, "
        "고치고, 제출하고, 판단하는 흐름입니다. 바뀐 것은 데이터 주소와 열 이름뿐입니다.")
